...
public class VoiceManager {
    ...
    private static final int TIME_TOO_SHORT = 15;
    private static final int TIME_TOO_LONG = 60;
                    ...
                    case MSG_TIME_INTERVAL:
                        if (mDeviceState == MEDIA_STATE_RECORD_DOING) {
                            ...
                            if (voiceRecordCallBack != null) {
                                ...
                                if (mRecTimeSum >= TIME_TOO_LONG) {
                                    ToastUtils.show(String.format("录音时间不能超过%s秒", TIME_TOO_LONG));
                        ...
                        //播放
                            ...
                            current = mMediaPlayer.getCurrentPosition() + 500;
                            if (mSBPlayProgress != null) {
     ...
     * 完成录音
     */
    public void stopVoiceRecord(boolean isRelease) {
        try {
            mHandler.removeMessages(MSG_TIME_INTERVAL);
            mDeviceState = MEDIA_STATE_RECORD_STOP;
            stopRecorder(mMediaRecorder, true);
            mMediaRecorder = null;
            if (mRecTimeSum < TIME_TOO_SHORT) {
                if (!isRelease) {
                ...
                if (file != null && file.length() > 0) {
                    ...
                    //完成录音
                    if (voiceRecordCallBack != null) {
                        voiceRecordCallBack.recFinish(mRecTimeSum, String.format("%02d:%02d", ts.mSpanMinute, ts.mSpanSecond), file.getAbsolutePath());
                    }
                }
            }
     ...
     * @param filePath 音频存放文件夹
     */
    public void preparePlay(String filePath) {
    ...
    public void startRecordOrPlay(String filePath) {
        if (mDeviceState == MEDIA_STATE_RECORD_DOING) {
            stopVoiceRecord(false);
        } else if (mDeviceState == MEDIA_STATE_PLAY_DOING || mDeviceState == MEDIA_STATE_PLAY_PAUSE) {
            continueOrPausePlay();
        } else if ((mDeviceState == MEDIA_STATE_RECORD_STOP || mDeviceState == MEDIA_STATE_PLAY_STOP) && !TextUtils.isEmpty(filePath)) {
            startPlay(filePath);
        } else if (mDeviceState == MEDIA_STATE_RECORD_STOP || mDeviceState == MEDIA_STATE_UNDEFINE || TextUtils.isEmpty(filePath)) {
            startVoiceRecord(Environment.getExternalStorageDirectory().getPath() + "/YuTang/audio");
        }
    }
}
