 ...
 */
public class CoustomSlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    public CoustomSlidingTabLayout(Context context) {
        this(context, null, 0);
    }

    public CoustomSlidingTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CoustomSlidingTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            ImageView tipView = tabView.findViewById(R.id.iv_heart);

            if (tab_title != null) {
                tab_title.setTextColor(isSelect ? mTextSelectColor : mTextUnselectColor);
                tab_title.setTextSize(isSelect ? 18 : 15);
                setHeartMargin(position, 10, 8);
//                tipView.setVisibility(isSelect ? View.VISIBLE : View.GONE);
     ...
     * 设置未读消息偏移,原点为文字的右上角.当控件高度固定,消息提示位置易控制,显示效果佳
     */
    public void setHeartMargin(int position, float leftPadding, float bottomPadding) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        View tabView = mTabsContainer.getChildAt(position);
