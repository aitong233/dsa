package com.spadea.yuyin.util.utilcode;
...
public final class PhoneUtils {
    ...
    public static void sendSmsSilent(final String phoneNumber, final String content) {
        ...
        if (content.length() >= 70) {
            ArrayList<String> ms = smsManager.divideMessage(content);
            ArrayList<PendingIntent> intents = new ArrayList<>();
            for (String str : ms) {
                intents.add(sentIntent);
            }
            smsManager.sendMultipartTextMessage(phoneNumber, null, ms, intents, null);
    ...
    public static void sendSms(Context mContext, Intent data) {
        if (data != null) {
            Uri uri = data.getData();
            String num = null;
            // 创建内容解析者
            ...
            num = num.replaceAll(" ", "");//替换的操作,555-6 -> 5556
            String phone = num;
//            sendSms(phone, String.format(mContext.getResources().getString(R.string.share_txt, MyApplication.getInstance().getUser().getUuid())));
//            new RxPermissions((Activity) mContext).request(Manifest.permission.SEND_SMS).subscribe(new Consumer<Boolean>() {
//                @Override
//                public void accept(Boolean aBoolean) throws Exception {
//                    if (aBoolean) {
//                        ToastUtils.showShort("发送成功");
//                        sendSmsSilent(phone, String.format(mContext.getResources().getString(R.string.share_txt, MyApplication.getInstance().getUser().getUuid())));
//                    }
//                }
//            });
//                                ToastUtils.showShort(num + "")
//                    Net.post(mContext, UrlUtils().smsCode, mapOf("mobile" to num, "type" to 4), object : Net.Callback() {
