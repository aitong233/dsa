...
import com.qpyy.room.bean.SvgaModel;
...
public class SvgaAnimationView extends ConstraintLayout {
    ...
    private Queue<SvgaModel> queue = new LinkedList<SvgaModel>();
    ...
    private void initView(Context context) {
        LayoutInflater.from(context).inflate(R.layout.room_view_svga_animation, this, true);
    ...
    private void showGift(SvgaModel model) {
        try {
            ...
            if (!TextUtils.isEmpty(model.url)) {
                if (model.approachUrl.endsWith("webp")) {
                    ImageUtils.loadImageView(model.url, svgaImageView);
                    return;
                }
    ...
    private void showApproach(SvgaModel model) {
        try {
            if (!TextUtils.isEmpty(model.approachUrl)) {
                if (model.approachUrl.endsWith("webp")) {
                    ImageUtils.loadImageView(model.approachUrl, svgaApproach);
                    return;
                }
                SVGAParser svgaParser = SVGAParser.Companion.shareParser();
