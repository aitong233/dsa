...
public class EaseEmojiconMenu extends EaseEmojiconMenuBase {
    ...
    public void init(List<EaseEmojiconGroupEntity> groupEntities) {
        ...
        mIvDelete.setOnTouchListener(new OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {

                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    mIvDelete.setImageResource(R.drawable.ease_ic_delete_emoji_red);
                    mIvDelete.postDelayed(mDeleteTask, 100);   //手指按下时触发不停的发送消息
                } else if (event.getAction() == MotionEvent.ACTION_UP) {
                    mIvDelete.setImageResource(R.drawable.ease_ic_delete_emoji_grey);
                    mIvDelete.removeCallbacks(mDeleteTask);  //手指抬起时停止发送
                }
                return false;
            }
        });
    ...
    private Runnable mDeleteTask = new Runnable() {
        ...
        public void run() {
            ...
            mIvDelete.postDelayed(this, 100);
        }
    };
