...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void setUpBaseView() {
        ...
        if (timestamp != null) {
            if (position == 0) {
                timestamp.setText(com.hyphenate.easeui.utils.DateUtils.getTimestampString(new Date(message.getMsgTime())));
            ...
            if (userAvatarView != null) {
                if (itemStyle.isShowAvatar()) {
                    ...
                    if (avatarOptions != null && userAvatarView instanceof EaseImageView) {
                        ...
                        avatarView.setBorderWidth(ConvertUtils.dp2px(2));
                        try {
                            if (UserBean.FEMALE.equals(message.getStringAttribute("sex"))) {
                                avatarView.setBorderColor(Color.parseColor("#FFFF8890"));
                            } else {
                                avatarView.setBorderColor(Color.parseColor("#FF97BAFF"));
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                            avatarView.setBorderColor(Color.parseColor("#FF97BAFF"));
            ...
            if (bubbleLayout != null) {
                if (message.direct() == Direct.SEND) {
                    if (itemStyle.getMyBubbleBg() != null) {
                        Drawable myBubbleBg = ((EaseMessageAdapter) adapter).getMyBubbleBg();
                        if (myBubbleBg==null){
                            myBubbleBg=itemStyle.getMyBubbleBg();
                        }
                        bubbleLayout.setBackground(myBubbleBg);
                    }
                } else if (message.direct() == Direct.RECEIVE) {
                    if (itemStyle.getOtherBubbleBg() != null) {
                        Drawable otherBubbleBg = ((EaseMessageAdapter) adapter).getOtherBubbleBg();
                        if (otherBubbleBg==null){
                            otherBubbleBg=itemStyle.getOtherBubbleBg();
                        }
                        bubbleLayout.setBackground(otherBubbleBg);
                    }
                }
            }
        }
     ...
     * setup view
     */
    protected abstract void onSetUpView() throws InterruptedException;
}
